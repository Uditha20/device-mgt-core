        ...
        try {
            ...
            if (DeviceManagementConstants.Report.DEVICE_LOGS.equals(operation.getCode())) {
                ...
                try {
                    ...
                    LogsDetailsWrapper logsWrapper = getWrapper(device, payloadArray);
...
//        }
    ...
    private static LogsDetailsWrapper getWrapper(Device device, JsonArray payloadArray) {
        List<LogsDetailsWrapper.LogPayload> payload = new ArrayList<>();
        for (JsonElement element : payloadArray) {
            JsonObject payloadObj = element.getAsJsonObject();
            LogsDetailsWrapper.LogPayload logPayload = new LogsDetailsWrapper.LogPayload();

            logPayload.setLogType(payloadObj.get("LOG_TYPE").getAsString());

            JsonArray logDataArray = payloadObj.getAsJsonArray("LOG_DATA");
            List<LogsDetailsWrapper.LogEntry> logData = new ArrayList<>();

            for (JsonElement logDataElement : logDataArray) {
                JsonObject logDataObj = logDataElement.getAsJsonObject();
                LogsDetailsWrapper.LogEntry entry = new LogsDetailsWrapper.LogEntry();
